...
Original file is located at
    https://colab.research.google.com/drive/1cJgYTragIWKmyvkNfJOQj9ddWk6KImaq

고쳐야 할 부분, 부동 소수점 깔끔하게 바꾸기(언어 확정하고 디버깅)

전반적 함수 정의

표를 그리기 위해서 필요한 파라미터
1. 표1(n회 시행시 뽑힌 횟수별 확률)
표시 되어야 하는 값들
- 확률 값
- 시도 횟수
- 뽑힌 횟수 별 확률
...
2. 표2(확률 별 최소 필요 시행 횟수)
- 확률 별 시행 횟수
ex) 1번 이상 50%이상 활률로 성공하기 위해서는 7회이상 시도하셔야 합니다.
...
def min_trial(p):
    prob_dic = get_prob_try(p)
    convert_to_decimal_point={5:0.05,10:0.1,15:0.15,20:0.2,25:0.25,30:0.3,35:0.35,40:0.4,45:0.45,50:0.5,55:0.55,60:0.6,65:0.65,70:0.7,75:0.75,80:0.8,85:0.85,90:0.9,95:0.95,99:0.99}
    min_required_trial_prob={}
    for k, v in convert_to_decimal_point.items():
        for try_num, prob in prob_dic.items():
            if v <= prob:
                min_required_trial_prob[k] = try_num
                break
    return min_required_trial_prob
...
def sampling_with_replacement(prob, try_count):
    ...
    min_required_trial_prob = min_trial(prob)
...
def min_trial_ceiling(p, limit):
    prob_dic = get_prob_try_ceiling(p, limit)
    convert_to_decimal_point={5:0.05,10:0.1,15:0.15,20:0.2,25:0.25,30:0.3,35:0.35,40:0.4,45:0.45,50:0.5,55:0.55,60:0.6,65:0.65,70:0.7,75:0.75,80:0.8,85:0.85,90:0.9,95:0.95,99:0.99,100:1}
    min_required_trial_prob={}
    for k, v in convert_to_decimal_point.items():
        for try_num, prob in prob_dic.items():
            if v <= prob:
...
def sampling_with_replacement_ceiling(prob, try_count, limit):
    ...
    min_required_trial_prob = min_trial_ceiling(prob, limit)
    return prob_of_picked, min_required_trial_prob
...
def min_trial_interval_same(range_list, prob_list):
    prob_dic = get_prob_try_interval_same(range_list, prob_list)
    convert_to_decimal_point={5:0.05,10:0.1,15:0.15,20:0.2,25:0.25,30:0.3,35:0.35,40:0.4,45:0.45,50:0.5,55:0.55,60:0.6,65:0.65,70:0.7,75:0.75,80:0.8,85:0.85,90:0.9,95:0.95,99:0.99}
    min_required_trial_prob={}
    for k, v in convert_to_decimal_point.items():
        for try_num, prob in prob_dic.items():
...
def prob_by_interval_same_item(range_list, prob_list, try_pick):
    ...
    prob_of_picked[0] = never_picked_same(range_list, prob_list, try_pick)
    min_required_trial_prob = min_trial_interval_same(range_list, prob_list)
    return prob_of_picked, min_required_trial_prob
...
def min_trial_interval_diff(start_count, prob):
    prob_dic = get_prob_try_interval_diff(start_count, prob)
    convert_to_decimal_point={5:0.05,10:0.1,15:0.15,20:0.2,25:0.25,30:0.3,35:0.35,40:0.4,45:0.45,50:0.5,55:0.55,60:0.6,65:0.65,70:0.7,75:0.75,80:0.8,85:0.85,90:0.9,95:0.95,99:0.99}
    min_required_trial_prob={}
    for k, v in convert_to_decimal_point.items():
        for try_num, prob in prob_dic.items():
...
def prob_by_interval_diff_item(start_count, prob, try_pick):
    ...
    prob_of_picked[0] = never_picked_diff(start_count, prob, try_pick)
    min_required_trial_prob = min_trial_interval_diff(start_count, prob)
    return prob_of_picked, min_required_trial_prob

table1, table2 = prob_by_interval_diff_item(100, 10, 110)
...
def complete_gacha(all_coupon_count, prob_list):
    ...
    prob_list = list(map(convert_to_decimal, prob_list))
...
complete_gacha(10,[10,10,10,10,10,10,10,10,10,10])