...
function clt() {
    ...
    // json 파일 받아오기 (ajax) 전달 Input probs
    ...
    function draw_probability() {
        ...
        bar
            .on("mouseover", tipProb_name.show)
            .on("mouseout", tipProb_name.hide);
    ...
    function draw_histogram() {
        ...
        if (flag && check_pop()) {
            ...
            document
                ...
                .innerText = " 모든 아이템이 뽑힐 때 까지 시도횟수 " + String(total_cnt) + "회, 총 금액 " +
                        String(total_cost) + "원 소비되었습니다.";
        }

        // update scale
        var ymax = d3.max(data_histo.map(function (d) {
            return d.y;
        }));
        y_scale_clt.domain([
            0, ymax * bins
        ...
        bar
            .on("mouseover", tipHisto_name.show)
            .on("mouseout", tipHisto_name.hide);
